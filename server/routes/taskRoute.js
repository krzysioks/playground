...
taskRouter.post('/task/login', async (req, res) => {
    ...
    try {
        ...
        const result = await UserModel.findByCredentials(username, password);
        //if user exists and password match -> send success response to front end
        res.status(200).send({ isUser: true, passwordMatched: true });
    } catch (err) {
